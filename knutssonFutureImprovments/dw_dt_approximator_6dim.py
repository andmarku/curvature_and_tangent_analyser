...
def dmDotTangents(tensor_knutVecs, tensor_tangents):
    ...
    Parameters
    ---------
    tensor_knutVecs : the flattened Knutsson vector in each voxel, N x M x L x 6.
    ...
    -------
    vector of floats
        dmAlongTangent : derivatives of flattened Knuttson vector, N x M x L x 6.
    '''
    tensor_DM = approximateAlong_XYZ(tensor_knutVecs)

    dim = tensor_knutVecs.shape
    dmAlongTangent = np.zeros((dim[0],dim[1],dim[2],6))
...
def approximateAlong_XYZ(knutVecs):
    ...
    Parameters
    ---------
    knutVecs : knutsson for every voxel, assumes knutVecs are N x M x L x 6.

    Returns
    -------
    vector of floats
        tensor_DM : dimensions N x M x L x 3 x 6.
                    N, M, L are the 3 dimensional space coordinates.
                    3 x 9 are the dimension of the local DM matrix.
    '''
    dim = knutVecs.shape
    tensor_DM = np.zeros((dim[0],dim[1],dim[2],3,6))

    for elementInKnut in range(6):
