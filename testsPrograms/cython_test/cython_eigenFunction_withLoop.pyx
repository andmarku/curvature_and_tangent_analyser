...
cpdef cnp.ndarray[double, ndim=4] calculateTangent(cnp.ndarray arg):

    dim = arg.shape

    cdef double [:,:,:, :, :] narr_view = arg

    data = np.zeros((dim[0], dim[1], dim[2], 3))

    cdef double [:, :, :, :] egVecs = data
    ...
    cdef double[3] egVec
    cdef cnp.ndarray[cnp.double_t, ndim=1] npEgVec
    ...
    for x in range(dim[0]):
      for y in range(dim[0]):
        for z in range(dim[0]):
          ...
          arg_as_array = [
             <double> narr_view[x,y,z,0,0], <double> narr_view[x,y,z,0,1], <double> narr_view[x,y,z,0,2],
             <double> narr_view[x,y,z,1,0], <double> narr_view[x,y,z,1,1], <double> narr_view[x,y,z,1,2],
             <double> narr_view[x,y,z,2,0], <double> narr_view[x,y,z,2,1], <double> narr_view[x,y,z,2,2]]

          # create the matrix (M - lambda I)
          smallestEgValue = calculateEigenValue(arg_as_array, arg)
          ...
          # normalize
          egVec = normalizeVector(raw_egVec)

          egVecs[x,y,z,:] = egVec
          # # cast to numpy array
          # npEgVec = np.asarray(egVec)
    data = np.asarray(egVecs)
